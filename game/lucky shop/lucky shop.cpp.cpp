...
void wn(){
	...
	int wnl[3]={0},wns[3];
	int myw,max=0,next=0,ms=0,ns=0;
	cout<<"欢迎来到蜗牛赛跑，请选择你赌的蜗牛！(1至3),胜利(100元),平局(50元)"<<endl<<"Welcome to the snail race, please choose the snail you bet on!(1 to 3),victory(100 yuan),tie(50 yuan)."<<endl;
	cin>>myw;
	for(int i=0;i<3;i++){
		wns[i]=rand()%3+1;
	}
	...
	if(ms>ns&&max==myw-1){
		v=true;
	}else if(ms==ns&&max==myw-1){
		t=true;
	}else if(ms==ns&&next==myw-1){
		t=true;
	}
	if(wns[0]==wns[1]&&wns[0]==wns[2]){
		t=true;
	}
	for(int i=0;wnl[0]<=15&&wnl[1]<=15&&wnl[2]<=15;i++){
		system("cls");
		for(int j=0;j<3;j++){
			wnl[j]+=wns[j];
			for(int k=0;k<wnl[j];k++){
				cout<<"     ";
			}
			cout<<j+1<<endl;
		}
...
int main(){
	...
	while(true){
		if(c=='y'){
			...
			cout<<"4.蜗牛竞速      4.Snail racing"<<endl;
